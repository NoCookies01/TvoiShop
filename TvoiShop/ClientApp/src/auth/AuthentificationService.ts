...
export class AuthorizeService {
    private _callbacks: { callback: VoidFunction, subscription: number }[] = [];
    ...
    async ensureUserManagerInitialized() {
        ...
        let response = await fetch("/_configuration/TvoiShop");
        if (!response.ok) {
            throw new Error(`Could not load settings for TvoiShop`);
        }
